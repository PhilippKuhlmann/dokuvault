...
class AgentTokenController extends Controller
{
    ...
    public function store(Customer $customer, Request $request)
    {
        ...
        $windowsClientScript = $this->windowsClientScript(url('/api/agent/windows-client'), $plain);
        ...
        return redirect(route('agent.index', $customer))
            ...
            ->with('windowsClientScript', $windowsClientScript);
    }
...
POWERSHELL;
    ...
    /**
     * Baut das Windows-Client-PowerShell-Script mit eingebettetem Endpoint und
     * Token. Laeuft auf jedem Windows-Arbeitsplatzrechner - anders als das
     * AD-Script ohne RSAT-Modul und ohne Domaincontroller-Voraussetzung.
     * MachineGuid als Identifier: pro Windows-Installation stabil und einzig,
     * ueberlebt auch einen Hostname-Wechsel.
     */
    protected function windowsClientScript(string $url, string $token): string
    {
        $script = <<<'POWERSHELL'
<#

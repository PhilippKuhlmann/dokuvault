...
- **Vordefinierte Demo-Zugänge sind geschützt**: Auf der Demo lassen sich `admin`, `techniker`, `kunde-rw` und `kunde-r` nicht löschen, und ihr Passwort und ihre Rolle nicht ändern. Vorher konnte der erste Besucher, der das Admin-Passwort änderte oder das Konto löschte, alle übrigen bis zum nächsten stündlichen Reset aussperren. Gesperrt sind vier Wege: Benutzerverwaltung (Löschen, Passwort, Rolle), eigenes Passwort im Profil und Löschen des eigenen Kontos. Selbst angelegte Benutzer bleiben uneingeschränkt bearbeitbar – die Benutzerverwaltung lässt sich also weiterhin ausprobieren. Außerhalb des Demo-Modus wirkt der Schutz nicht.

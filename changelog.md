...
- **Globale Suche und Kundensuche per Tastatur bedienbar.** Pfeil hoch/runter bewegt die Auswahl über alle Treffer (in der globalen Suche bereichsübergreifend), Enter öffnet den markierten – ohne die Hand von der Tastatur zu nehmen. Der erste Treffer ist gleich markiert; beim Weitertippen springt die Auswahl zurück an den Anfang. Esc schließt die Suche (zurück zur vorigen Seite); ein kleiner Hinweis „↑↓ wählen · ↵ öffnen · Esc schließen" steht unter dem Feld.
- **Globale Suche springt zum Treffer.** Ein Klick auf ein Suchergebnis öffnet nicht mehr nur die Liste, sondern scrollt direkt zum gefundenen Gerät und hebt es kurz hervor. Der blaue Rahmen liegt dabei genau an der Karte (folgt ihren runden Ecken), nicht am unsichtbaren Wrapper darum herum. Liegt der Treffer auf einer späteren Blätter-Seite, springt die Liste dorthin; ein in der Seitenleiste gewählter Standort wird für diesen einen Sprung ausgesetzt, damit der Treffer nicht ausgeblendet bleibt. (Der Treffer wird über den Query-Parameter `?highlight=` übergeben.)
...
- **Assistent-Felder wurden nach „Hinzufügen" nicht immer geleert.** Nach dem Anlegen blieb der gerade getippte Name gelegentlich stehen – ein zweiter Klick hätte denselben Eintrag noch einmal angelegt. Ursache war eine Reihenfolge: Livewire feuert das Leeren-Event, morpht das DOM aber unmittelbar danach und stellt die als „dirty" geschützten (eben getippten) Eingaben wieder her. Das Leeren läuft jetzt in `$nextTick`, also nach dem Morph. (Beim kompletten Durchtesten des Assistenten aufgefallen.)

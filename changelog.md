...
- **Schon erfasste Einträge im Assistenten direkt bearbeiten.** Ein Klick auf einen „Schon erfasst"-Eintrag öffnet ihn jetzt im Bearbeiten-Modal direkt im Assistenten, statt in einem neuen Tab die Liste zu öffnen – mit allen Feldern, derselben Validierung und Verschlüsselung wie überall sonst. Es ist genau dieselbe config-getriebene `ObjektFormular`-Komponente, die auch die Listen benutzen (per Event `objekt-bearbeiten` geöffnet, nach dem Speichern lädt der Assistent die Liste neu); ein Schalter blendet dort ihren eigenen „Neu"-Knopf aus, weil der Assistent selbst anlegt. Gilt für 17 der 18 Schritte; nur „Netzwerk" hat kein passendes Modal und behält den Listen-Link im neuen Tab. Ohne Änderungsrecht bleibt es beim Link. Vier neue Tests.

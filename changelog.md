...
- **Der UniFi-Agent hält fest, ob ein Gerät seine Adresse per DHCP bezieht.** Eine geliehene Adresse sah in der Doku aus wie eine feste — und nach dem nächsten Stromausfall stand dort etwas Falsches. Jetzt trägt die Adresse die Bezeichnung „DHCP"; im IPAM steht dann „AP-Büro (DHCP)", in der Geräteliste in der Spalte Bezeichnung.
  - Es sitzt an der Adresse, nicht am Gerät: Ein Switch kann mehrere haben.
  - Wird aus DHCP eine feste Adresse, verschwindet die Marke wieder — sonst behauptet die Doku dauerhaft etwas, das nicht mehr stimmt. Eine selbst geschriebene Bezeichnung wie „Uplink Dachboden" bleibt dagegen unangetastet, und ältere Firmware, die dazu nichts meldet, ändert gar nichts.

...
- **Anlegen und Bearbeiten im Modal — Grundstein für alle Typen.** Das VLAN hatte es vorgemacht: Wer einen Eintrag nachträgt, verliert sonst die Liste, auf die er gerade geschaut hat. Statt das Muster vierzigmal zu kopieren (VLAN kostete rund 630 Zeilen), beschreibt jetzt `config/forms.php` die Felder, und **eine** Livewire-Komponente baut das Formular daraus. Umgestellt sind **29 Typen** — neben den einfachen jetzt auch die Geräte: Accesspoint, Kamera, Computer, DECT, IoT-Gerät, NAS, Switch, Sonstiger Client, Telefon, TK-Anlage, Drucker, Recorder, Router, USV und VM.

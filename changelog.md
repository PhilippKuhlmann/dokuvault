...
- **Docker-Setup**: `docker compose up` startet DokuVault samt MariaDB auf http://localhost:8000, mit Demo-Daten und Zugängen aus dem ersten Start. Vorher brauchte jeder Interessent PHP, Composer, Node und eine Datenbank, bevor er auch nur hineinsehen konnte. Geseedet wird nur, wenn die Datenbank leer ist – ein Neustart verdoppelt die Daten also nicht. Gedacht zum Ausprobieren und für kleine Installationen: ein Container, ein Prozess, kein nginx. Eine eigene GitHub-Action baut das Image bei jedem Push, startet es samt Datenbank und meldet sich erst grün, wenn eine Anmeldung mit dem Demo-Zugang eine angemeldete Sitzung mit Demo-Daten ergibt. Die Zugangsdaten schreibt der Start in die `.env`, statt sich auf die Umgebung zu verlassen: `php artisan serve` reicht nur eine feste Liste von Variablen an den Serverprozess weiter, `DB_*` gehört nicht dazu.
- **Vordefinierte Demo-Zugänge sind gesperrt**: Auf der Demo lassen sich `admin`, `techniker`, `kunde-rw` und `kunde-r` weder löschen noch ändern – Passwort, Rolle, E-Mail und auch der Benutzername stehen fest. Vorher konnte der erste Besucher, der das Admin-Passwort änderte oder das Konto löschte, alle übrigen bis zum nächsten stündlichen Reset aussperren. Dass der Benutzername mitgesperrt ist, hat einen eigenen Grund: Der Schutz erkennt die Zugänge daran, eine Umbenennung hätte ihn aufgehoben. Statt eines Formulars, dessen Eingaben verworfen werden, zeigt die Bearbeiten-Seite die Daten nur noch an. Gesperrt sind vier Wege: Benutzerverwaltung (Ändern und Löschen), eigenes Passwort im Profil und Löschen des eigenen Kontos. Selbst angelegte Benutzer bleiben uneingeschränkt bearbeitbar – die Benutzerverwaltung lässt sich also weiterhin ausprobieren. Außerhalb des Demo-Modus wirkt der Schutz nicht.
- **Nutzung der Demo auswerten**: Bei `DEMO_MODE=true` hält die App fest, wann die Demo besucht wurde und mit welcher Rolle. `php artisan demo:stats` zeigt Besuche und Seitenaufrufe gesamt, je Tag, über die Tageszeit und je Rolle. Bewusst ohne Personenbezug – keine IP-Adresse, kein User-Agent, keine aufgerufenen Seiten; Besuche werden über einen Zufallswert in der Sitzung unterschieden. Die Aufzeichnung liegt als Datei unter `storage/app/demo-usage/`, weil der stündliche Reset die Datenbank komplett leert.
- **Patchfelder mit Dosendokumentation**: Unter „Netzwerk → Patchfelder" lassen sich Patchfelder anlegen (Standort, Portanzahl, Höheneinheiten, Hersteller). Die Portzeilen entstehen dabei automatisch; je Port trägt man die Dosennummer („EG 1.01", „A.12", „2.23"), den Raum und den Ziel-Switch samt dessen Port ein. Dosennummer und Raum sind getrennte Felder – so lässt sich gezielt nach der Nummer suchen, und Listen stehen sauber untereinander. Damit beantwortet die Doku die Frage, für die man sonst in den Schrank schaut: „Dose 2.14 im Besprechungsraum – wo hängt die?". Die **globale Suche findet Dosen** über ihre Bezeichnung, das Patchfeld ist im **Serverschrank einbaubar** und der **PDF-Export** bekommt einen Abschnitt samt Dosenbelegung je Feld. Papierkorb und Standortfilter gelten wie bei allen Objekten. Die Portanzahl lässt sich nachträglich erhöhen; verkleinern lehnt die App ab, solange oberhalb noch dokumentierte Ports liegen – sie nennt die betroffenen Portnummern. Bestehende Installationen erhalten die vier neuen Berechtigungen per Migration.

### Changed

- **Anleitung zum Aktualisieren**: Bisher beschrieben README und DEPLOYMENT.md nur die Neuinstallation und den automatischen Deploy. Wer DokuVault selbst betreibt und auf einen neueren Stand will, findet die Schritte jetzt unter „Aktualisieren" – mit Backup vorab, und mit dem Wartungsfenster nur um die Migrationen herum statt um den ganzen Vorgang.
- **Tests laufen gegen mehrere PHP-Versionen und gegen MariaDB**: Die CI prüfte bisher nur PHP 8.2 mit SQLite, obwohl `composer.json` `^8.2` verspricht und produktiv MySQL/MariaDB läuft. Jetzt PHP 8.2 und 8.3 jeweils mit SQLite und MariaDB 11; PHP 8.4 läuft mit, blockiert aber nicht.
- **Der vertrauenswürdige Proxy steht in der Konfiguration**: Bisher war er in `TrustProxies.php` fest verdrahtet. Welcher Reverse-Proxy vor der App steht, ist aber eine Eigenschaft der Installation – jetzt über `TRUSTED_PROXIES` in der `.env`, mehrere Einträge und CIDR erlaubt. Ohne passenden Eintrag sieht die App die Adresse des Proxys statt der des Besuchers; das betrifft die Herkunftsstatistik und alles andere, was `request()->ip()` nutzt.
- **Die Demo-Statistik zeigt die Herkunft**: `demo:stats` gibt jetzt aus, aus welchen Netzen die Besuche kamen. Standardmäßig wird die Adresse dafür auf das Netz gekürzt (/24 bei IPv4, /48 bei IPv6) – das beantwortet die Frage nach dem Woher genauso, weil eine GeoIP-Abfrage aus `91.65.42.0` dasselbe Land liefert wie aus der vollen Adresse, ist aber keinem Anschluss mehr zuzuordnen. Über `DEMO_IP_LOGGING` lässt sich auf `aus` oder `voll` umstellen; `voll` speichert ein personenbezogenes Datum und gehört dann in die Datenschutzerklärung. Sind die aufgezeichneten Adressen nicht öffentlich, weist der Befehl darauf hin, dass vermutlich ein Proxy davorsteht und seine Adresse in `TrustProxies` fehlt – sonst läse man dessen Adresse als Ergebnis.
- **Deploy schaltet die Demo nur noch für Sekunden ab**: Bisher lag der Wartungsmodus über dem gesamten Deploy – auch über `composer install` und den Frontend-Build, die keine Auszeit brauchen. Abgeschaltet wird jetzt erst für Migrationen, Cache-Neuaufbau und den Demo-Reset. Auf der Demo gemessen: 10 Sekunden statt 17, und dieser Abstand wächst deutlich, sobald sich Abhängigkeiten ändern – dann dauern Composer und der Frontend-Build Minuten statt Sekunden. Außerdem nimmt `deploy.sh` eine Dateisperre, die sich der stündliche `demo:reset`-Cronjob teilt (`flock -n`, siehe DEPLOYMENT.md): vorher konnte der Reset mitten in einen Deploy fallen und `migrate` gegen eine Datenbank laufen, die gerade geleert wurde.

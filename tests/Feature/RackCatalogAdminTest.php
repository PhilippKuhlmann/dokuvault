...
test('Anlegen scheitert ohne Bezeichnung und bei ungültiger Höhe', function () {
    ...
    $this->post('/admin/rackcatalogitem/create', ['height_units' => 1])
        ->assertSessionHasErrors('name');

    $this->post('/admin/rackcatalogitem/create', ['name' => 'Zu hoch', 'height_units' => 43])
        ->assertSessionHasErrors('height_units');

    $this->post('/admin/rackcatalogitem/create', ['name' => 'Zu niedrig', 'height_units' => 0])
        ->assertSessionHasErrors('height_units');
...
test('Bezeichnung muss eindeutig sein, der eigene Datensatz aber erlaubt', function () {
    ...
    $this->post('/admin/rackcatalogitem/create', ['name' => 'Rangierfeld', 'height_units' => 1])
        ->assertSessionHasErrors('name');
...
test('Nicht-Admins kommen an den Katalog nicht heran', function () {
    ...
    $this->post('/admin/rackcatalogitem/create', ['name' => 'X', 'height_units' => 1])->assertStatus(403);
    $this->patch("/admin/rackcatalogitem/{$eintrag->id}", ['name' => 'Y', 'height_units' => 1])->assertStatus(403);

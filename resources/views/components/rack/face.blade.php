...
<svg viewBox="0 0 1086 {{ $h }}" preserveAspectRatio="none" role="presentation" aria-hidden="true"
    ...
    @for ($u = 0; $u < $he; $u++)
        ...
        <circle cx="23" cy="{{ $cy }}" r="9" fill="none" stroke="currentColor" stroke-opacity="0.5" stroke-width="3" />
        <circle cx="1063" cy="{{ $cy }}" r="9" fill="none" stroke="currentColor" stroke-opacity="0.5" stroke-width="3" />
    @endfor
    <line x1="{{ $inset }}" y1="6" x2="{{ $inset }}" y2="{{ $h - 6 }}" stroke="currentColor" stroke-opacity="0.25" stroke-width="2" />
    <line x1="{{ 1086 - $inset }}" y1="6" x2="{{ 1086 - $inset }}" y2="{{ $h - 6 }}" stroke="currentColor" stroke-opacity="0.25" stroke-width="2" />
    ...
    @switch($appearance)
        ...
        @case('nas')
            ...
            @for ($u = 0; $u < $he; $u++)
                @for ($i = 0; $i < $bays; $i++)
                    ...
                    <rect x="{{ $x + 3 }}" y="{{ $y }}" width="{{ $bayW - 6 }}" height="56" rx="4"
                        fill="currentColor" fill-opacity="0.10" stroke="currentColor" stroke-opacity="0.4" stroke-width="2" />
                    {{-- Griffmulde --}}
            ...
            @endfor
            {{-- Power-Taste + Status --}}
            <circle cx="1020" cy="{{ $mid }}" r="13" fill="none" stroke="currentColor" stroke-opacity="0.5" stroke-width="3" />
            <line x1="1020" y1="{{ $mid - 12 }}" x2="1020" y2="{{ $mid - 2 }}" stroke="currentColor" stroke-opacity="0.5" stroke-width="3" />
            @break
            ...
            @for ($row = 0; $row < 2; $row++)
                @for ($i = 0; $i < $perRow; $i++)
                    ...
                    <rect x="{{ $x }}" y="{{ $y }}" width="{{ $portW }}" height="{{ $portH }}" rx="3"
                        fill="currentColor" fill-opacity="0.10" stroke="currentColor" stroke-opacity="0.45" stroke-width="2" />
                    {{-- RJ45-Nase --}}
        ...
        @case('router')
            @for ($i = 0; $i < 5; $i++)
                ...
                <rect x="{{ $x }}" y="{{ $mid - 17 }}" width="54" height="34" rx="3"
                    fill="currentColor" fill-opacity="0.10" stroke="currentColor" stroke-opacity="0.45" stroke-width="2" />
                <rect x="{{ $x + 19 }}" y="{{ $mid + 9 }}" width="16" height="8" fill="currentColor" fill-opacity="0.35" />
            ...
            @for ($i = 0; $i < 4; $i++)
                <circle cx="{{ 620 + $i * 34 }}" cy="{{ $mid }}" r="8"
                    fill="{{ $i === 0 ? '#22c55e' : 'currentColor' }}" fill-opacity="{{ $i === 0 ? 0.85 : 0.25 }}" />
            @endfor
            <rect x="800" y="{{ $mid - 22 }}" width="220" height="44" rx="6"
                fill="currentColor" fill-opacity="0.10" stroke="currentColor" stroke-opacity="0.35" stroke-width="2" />
            @break

        @case('ups')
            {{-- Display --}}
            <rect x="80" y="{{ $mid - min(34, $h / 2 - 12) }}" width="300" height="{{ min(68, $h - 24) }}" rx="6"
                fill="currentColor" fill-opacity="0.22" stroke="currentColor" stroke-opacity="0.45" stroke-width="2" />
            @for ($i = 0; $i < 3; $i++)
                <rect x="{{ 100 + $i * 58 }}" y="{{ $mid - 8 }}" width="42" height="16" rx="2" fill="currentColor" fill-opacity="0.4" />
            @endfor
            {{-- Bedientasten --}}
            @for ($i = 0; $i < 2; $i++)
                <circle cx="{{ 430 + $i * 54 }}" cy="{{ $mid }}" r="16"
                    fill="currentColor" fill-opacity="0.12" stroke="currentColor" stroke-opacity="0.45" stroke-width="2" />
            @endfor
            {{-- Lueftung --}}
            @for ($c = 0; $c < 18; $c++)
            ...
            @for ($u = 0; $u < $he; $u++)
                ...
                @for ($i = 0; $i < $ports; $i++)
                    ...
                    <rect x="{{ $x + 2 }}" y="{{ $cy - 20 }}" width="{{ $pw - 4 }}" height="40" rx="3"
                        fill="currentColor" fill-opacity="0.10" stroke="currentColor" stroke-opacity="0.45" stroke-width="2" />
                    <rect x="{{ $x + $pw / 2 - 7 }}" y="{{ $cy + 12 }}" width="14" height="8" fill="currentColor" fill-opacity="0.35" />
                @endfor
                {{-- Beschriftungsstreifen (leer, ohne Text) --}}
                <rect x="70" y="{{ $cy + 26 }}" width="930" height="10" rx="2" fill="currentColor" fill-opacity="0.18" />
            @endfor
            @break
        ...
        @case('cablering')
            @for ($i = 0; $i < 5; $i++)
                ...
                <path d="M {{ $x }} {{ $mid + 26 }} v -18 a 34 34 0 0 1 68 0 v 18"
                    fill="none" stroke="currentColor" stroke-opacity="0.5" stroke-width="7" stroke-linecap="round" />
            @endfor
        ...
        @case('brush')
            ...
            <rect x="70" y="{{ $mid - 22 }}" width="930" height="44" rx="6"
                fill="currentColor" fill-opacity="0.28" stroke="currentColor" stroke-opacity="0.4" stroke-width="2" />
            @for ($i = 0; $i < 62; $i++)
        ...
        @case('shelf')
            ...
            <rect x="{{ $inset + 8 }}" y="{{ $mid - 26 }}" width="{{ 1086 - 2 * $inset - 16 }}" height="52" rx="4"
                fill="currentColor" fill-opacity="0.20" stroke="currentColor" stroke-opacity="0.4" stroke-width="2" />
            <line x1="{{ $inset + 8 }}" y1="{{ $mid - 8 }}" x2="{{ 1078 - $inset }}" y2="{{ $mid - 8 }}"
                stroke="currentColor" stroke-opacity="0.28" stroke-width="3" />
            @for ($i = 0; $i < 3; $i++)
        ...
        @case('pdu')
            @for ($i = 0; $i < 8; $i++)
                ...
                <circle cx="{{ $cx }}" cy="{{ $mid }}" r="30"
                    fill="currentColor" fill-opacity="0.12" stroke="currentColor" stroke-opacity="0.45" stroke-width="2" />
                <circle cx="{{ $cx - 11 }}" cy="{{ $mid }}" r="5" fill="currentColor" fill-opacity="0.5" />
                <circle cx="{{ $cx + 11 }}" cy="{{ $mid }}" r="5" fill="currentColor" fill-opacity="0.5" />
            @endfor
            <rect x="1000" y="{{ $mid - 18 }}" width="36" height="36" rx="4"
                fill="#ef4444" fill-opacity="0.55" stroke="currentColor" stroke-opacity="0.4" stroke-width="2" />
            @break

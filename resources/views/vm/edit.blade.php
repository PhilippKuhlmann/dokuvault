<x-app-layout :$customer>
    <x-create.main :header="__('VM bearbeiten')" :labelsubmit="__('Stammdaten speichern')" action="{{ route('vm.update', [$customer, $vm]) }}" breit>
        ...
        <x-create.abschnitt :titel="__('Identität')" erste
            x-data="{ host: '{{ old('server_id', $vm->server_id) }}', cluster: '{{ old('cluster_id', $vm->cluster_id) }}' }">
            ...
            <div x-show="! host" x-cloak class="flex flex-col mt-2">
                ...
                <x-input.select id="cluster_id" name="cluster_id" x-model="cluster">
                    ...
                    @foreach ($clusters as $cluster)
                        <option value="{{ $cluster->id }}" {{ $cluster->id == $vm->cluster_id ? 'selected' : '' }}>{{ $cluster->name }}</option>
                    @endforeach
                </x-input.select>
            </div>

...
<x-create.singlerow label="Bezeichnung" name="name" :default="$item->name ?? ''" />

<x-create.doublerow
    label1="Höheneinheiten (HE)" name1="height_units" type1="number" :default1="$item->height_units ?? 1"
    label2="Reihenfolge in der Palette" name2="sort_order" type2="number" :default2="$item->sort_order ?? 0" />

<div class="flex flex-col mt-2" x-data="{ appearance: '{{ $selected }}' }">
    <x-input.label for="appearance" value="Darstellung in der Frontansicht" />
    ...
    <p class="mt-2 text-xs text-gray-400 dark:text-gray-500">
        Vorschau mit 2 HE – die Zeichnung passt sich der eingestellten Höhe an.
    </p>

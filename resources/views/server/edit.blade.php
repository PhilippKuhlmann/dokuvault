<x-app-layout :$customer>
    <x-create.main :header="__('Server bearbeiten')" :labelsubmit="__('Speichern')" action="{{ route('server.update', [$customer, $server]) }}">
        @method('PATCH')
        ...
        <x-create.doublerow :label1="__('Hersteller')" name1="manufacturer" :default1="$server->manufacturer" :label2="__('Model')" name2="model" :default2="$server->model" />
        ...
        <x-create.doublerow :label1="__('IP 1')" name1="ip1" :default1="$server->ip1" :label2="__('IP 2')" name2="ip2" :default2="$server->ip2" />

        <x-create.singlerow :label="__('BMC IP')" name="bmcIp" :default="$server->bmcIp" />

        <x-create.doublerow :label1="__('BMC User')" name1="bmcUser" :default1="$server->bmcUser" :label2="__('BMC Passwort')" name2="bmcPassword" :default2="$server->bmcPassword" />

        <x-create.doublerow :label1="__('Rustdesk ID')" name1="remoteID" :default1="$server->remoteID" :label2="__('Rustdesk Passwort')" name2="remotePassword" :default2="$server->remotePassword" />

        <x-edit.select.operatingsystem selector="{{ $server->operatingSystem?->id }}" :$operatingSystems/>

        <x-create.singlerow :label="__('Dienste Bitte mit komma getrennt angeben (eins,zwei,drei)')" name="services" :default="implode(',', $server->services)" />
    ...
    <livewire:device-credentials :model="$server" :customer="$customer" />
